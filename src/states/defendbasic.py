...
class Defend(State):
    ...
    def execute(self, game_info):
        ...
        east_multiplier = util.sign(game_info.ball.location.x - game_info.me.location.x)
        # aim for side of the ball
        aim_location = game_info.ball.location + Vec3(east_multiplier * util.BALL_RADIUS, 0, 0)
        target_location = relative_location(game_info.ball.location, game_info.me.rotation, aim_location)
